if [ $# -ne 4 ]; then 
	echo "Usage: 4 arguments-> switchKey, propertyName, version, originserverhostname"
	exit 1
fi	
...
if [ "$var1" = '"origin"' ]
then 
	cat file.json | jq --arg key1 $num --arg key2 $originserverhostname '.rules.behaviors[$key1|tonumber].options.hostname=$key2 ' \
