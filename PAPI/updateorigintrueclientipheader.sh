if [ $# -ne 4 ]; then
        echo "Usage: 4 arguments-> switchKey, propertyName, version, TrueClientIP header name(no spaces or special characters)"
        exit 1
fi
...
if [ "$var1" = '"origin"' ]
then 
	cat file.json | jq --arg key1 $num --arg key2 "$trueclientipheader" '.rules.behaviors[$key1|tonumber].options.trueClientIpHeader=$key2 ' \
	> file.json.tmp && mv file.json.tmp file.json
fi
done

echo "Updating the True Client IP header name..."
